...
Güncel payload sadece token/timezone bilgisi taşımaz; registration health alanları da gönderir:

- `lastPushSyncAttemptAt`
- `lastPushSyncSuccessAt`
- `lastPushSyncFailureReason`
- `lastPushTokenHash`
- `hasPushToken`
- `adTelemetry` (cihaz tarafında toplanan yakın dönem reklam funnel/suppress özeti)
...
Bu alanlar Firestore `devices` dokümanına merge edilir ve admin panelde registration health/coverage özetlerinde kullanılır.
...
### Self-Healing Kayit Davranisi

Push registration akisi artik explicit self-healing davranis izler:

1. Uygulama acilisinda sync dener
2. `onNewToken` geldiginde tekrar sync dener
3. Basarisizlikta one-time retry worker schedule edilir
4. Periodic repair worker stale kayitlari tekrar dener

Developer mode acikken Ayarlar ekraninda:

- son basarili push sync zamani
- son hata nedeni
- `Push kaydini simdi yeniden dene`

alanlari gorulebilir.

Bu UI normal kullanicilara acik degildir.
